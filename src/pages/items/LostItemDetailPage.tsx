...
import { MapPin, Calendar, Tag, Palette, Award, Trash2, Edit, CheckCircle, ArrowLeft, Package, Sparkles } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';
import toast from 'react-hot-toast';
import { lostItemsApi } from '@/api/lostItems';
import { matchesApi } from '@/api/matches';
import type { LostItemDto, MatchSuggestionDto } from '@/types';
...
export default function LostItemDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        else navigate('/lost-items');
      } catch { navigate('/lost-items'); }
      setLoading(false);
    })();
  }, [id, navigate]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.success) { toast.success('Item deleted'); navigate('/lost-items'); }
  ...
  const handleRecover = async () => {
    if (!id) return;
    try {
      const res = await lostItemsApi.recover(id);
      if (res.success) { toast.success('Marked as recovered!'); setItem((prev) => prev ? { ...prev, status: 'Recovered' } : prev); }
      else toast.error(res.message);
  ...
  const loadSuggestions = async () => {
    ...
    try {
      const res = await matchesApi.getSuggestions(id);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {item.locationLost && (
              <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                <MapPin className="h-4 w-4" /> {item.locationLost}
              </div>
            )}
            {item.lostDate && (
              <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                <Calendar className="h-4 w-4" /> Lost on {new Date(item.lostDate).toLocaleDateString()}
              </div>
            )}
            {item.color && (
              <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            {item.rewardAmount != null && item.rewardAmount > 0 && (
              <div className="flex items-center gap-2 text-sm text-emerald-600 font-medium">
                <Award className="h-4 w-4" /> ${item.rewardAmount} Reward
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-2 pt-4 border-t border-gray-200 dark:border-gray-800">
            {isOwner && item.status === 'Active' && (
              <>
                <Button onClick={loadSuggestions} variant="outline" className="w-full gap-2">
                  <Sparkles className="h-4 w-4" /> Find Matches
                </Button>
                <Button onClick={handleRecover} variant="secondary" className="w-full gap-2">
                  <CheckCircle className="h-4 w-4" /> Mark Recovered
                </Button>
                <Link to={`/lost-items/${item.id}/edit`} className="w-full">
      ...
      {/* Suggestions modal */}
      <Modal open={showSuggestions} onClose={() => setShowSuggestions(false)} title="Match Suggestions" maxWidth="max-w-2xl">
        {sugLoading ? (
          <Spinner text="Finding matches..." />
        ) : suggestions.length === 0 ? (
          <p className="text-center text-sm text-gray-500 py-8">No matching found items yet. Check back later!</p>
        ) : (
          <div className="space-y-3">
            {suggestions.map((s) => (
              <div key={s.foundItemId} className="flex gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
                <div className="w-20 h-20 shrink-0 rounded-lg bg-gray-100 dark:bg-gray-800 overflow-hidden">
                  {s.imageUrls?.[0] ? (
                    <img src={s.imageUrls[0]} alt="" className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-gray-400"><Package className="h-8 w-8" /></div>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <h4 className="font-medium text-gray-900 dark:text-gray-100 truncate">{s.foundItemTitle}</h4>
                    <Badge variant="info">{Math.round(s.matchScore * 100)}% match</Badge>
                  </div>
                  <p className="text-xs text-gray-500 mt-1 line-clamp-2">{s.foundItemDescription}</p>
                  <p className="text-xs text-emerald-600 mt-1">{s.matchReason}</p>
                  <Link to={`/found-items/${s.foundItemId}`}>
                    <Button variant="ghost" size="sm" className="mt-1">View item</Button>
                  </Link>
                </div>
              </div>
            ))}
