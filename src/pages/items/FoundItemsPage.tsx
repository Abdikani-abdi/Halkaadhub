...
export default function FoundItemsPage() {
  ...
  const [items, setItems] = useState<FoundItemDto[]>([]);
  ...
  const fetchItems = async (p: number, q: string) => {
    ...
    try {
      const res = q
        ? await foundItemsApi.search({ query: q, page: p, pageSize: 12 })
        : await foundItemsApi.getAll(p, 12);
      setItems(res.data || []);
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Found Items</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Browse items reported as found</p>
        </div>
        <Link to="/items/new?type=found">
          <Button className="gap-1.5"><Plus className="h-4 w-4" />Report Found Item</Button>
        </Link>
      </div>
      ...
      {loading ? (
        <Spinner text="Loading found items..." />
      ) : items.length === 0 ? (
        <EmptyState
          title="No found items found"
          description={query ? 'Try adjusting your search terms.' : 'No found items have been reported yet.'}
          action={
            <Link to="/items/new?type=found">
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-3 mt-3 text-xs text-gray-400">
                    {item.locationFound && (
                      <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{item.locationFound}</span>
                    )}
                    {item.categoryName && (
