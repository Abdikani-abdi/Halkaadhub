...
export default function AdminFoundItemsPage() {
  ...
  const load = async (p: number) => {
    setLoading(true);
    try {
      const res = await foundItemsApi.search({ page: p, pageSize: 20 });
      setItems(res.data || []);
  ...
  return (
    <div>
      <PageHeader
        title="Found Items"
        description="Manage all found item reports"
        action={
          <div className="flex items-center gap-3">
                ...
                <tr>
                  <td colSpan={6} className="px-5 py-16">
                    <EmptyState
                      ...
                      description={items.length === 0 ? "No found items have been reported yet." : "Try adjusting your search filters."}
                    />
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={item.id} className="group hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        {item.imageUrls && item.imageUrls.length > 0 ? (
                          ...
                          <div className="h-12 w-12 rounded-xl bg-emerald-100 dark:bg-emerald-500/20 flex items-center justify-center ring-2 ring-gray-100 dark:ring-gray-800">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <span className="truncate max-w-[150px]">{item.locationFound || '—'}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4">
